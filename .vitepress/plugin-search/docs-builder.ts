...
import {slugify} from "@mdit-vue/shared"  // Use same slugify fn as Vitepress so ids always match
...
const removeAnyTagOrVariable = (md: string): string => {
    // Remove tags but not their contents (unlike script/style removal) and rm variables
    return md.replace(/<.+?>/gi, '').replace(/\{\{.+?\}\}/g, '').trim()
}
...
const processMdFiles = async (dirName: string): Promise<mdFiles[]> => {
    ...
    for (let index = 0; index < mdFilesList.length; index++) {
        ...
        let cleanCode = removeAnyTagOrVariable(removeStyleTag(
...
const buildDoc = (mdDoc: MdIndexDoc, id: string): Doc => {
    ...
    let link = mdDoc.path.replace(rootPath + "/", "").replace("index.md", "")
